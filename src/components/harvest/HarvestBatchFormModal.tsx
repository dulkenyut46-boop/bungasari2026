...
import { formatRupiah, formatNumber, formatKg } from '../../lib/utils';
...
export const HarvestBatchFormModal: React.FC<HarvestBatchFormModalProps> = ({
  ...
  const { farmers, addHarvestBatch, updateHarvestBatch, currentUser } = useApp();
  ...
  useEffect(() => {
    if (batchToEdit) {
      ...
      // Initialize with first 5 farmers as initial rows
      if (farmers.length > 0) {
        const initialRows: HarvestFarmerDetail[] = farmers.slice(0, 4).map((f, idx) => ({
          farmerId: f.id,
          farmerName: f.name,
          bunchCount: 100 + idx * 15,
  ...
  // Add new farmer row
  const handleAddRow = () => {
    // Pick an unused farmer if possible
          ...
          <div className="flex items-center justify-between pb-2">
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Total TPH: <span className="font-bold text-slate-900 dark:text-white">{formatKg(totalTphWeightKg)}</span> ({totalBunches} Janjang)
              </p>
            </div>
            <button
              type="button"
            ...
            >
              <Plus className="h-3.5 w-3.5" /> Tambah Baris Petani
            </button>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="sticky top-0 bg-slate-100 text-[11px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300 z-10 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="p-2.5 w-24">Janjang</th>
                  <th className="p-2.5 w-32">Berat TPH (Kg)</th>
                  <th className="p-2.5 text-right">Hak Petani</th>
                ...
                {items.map((item, index) => (
                  <tr key={index} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="p-2">
                      <input
                        type="number"
                        value={item.bunchCount}
                        onChange={e => handleItemChange(index, 'bunchCount', Number(e.target.value))}
                        className="w-full rounded border border-slate-200 bg-white p-1 text-xs dark:border-slate-700 dark:bg-slate-800"
                      />
                    </td>
                    <td className="p-2">
